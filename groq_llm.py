...
"""
    try:
        ...
        if not text:
            logger.error(f"Groq OSS 120B returned no text. Full response: {json.dumps(data, indent=2)[:300]}")
        ...
        if lines:
            ...
            return {"status": "ok", "tests": lines[:max_cases], "reason": "Groq OSS 120B test generation successful"}
